...
import deletarCredito from './requests/deleteCredit'
import BorderColorIcon from '@mui/icons-material/BorderColor';
import EditarCreditoModal from '../Edit/EditarCreditoModal'
...
export default function CreditTable(props) {
    const billingCycle = useSelector((state) => state.ciclos.billingCycleAtual.creditos)
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - creditos.length) : 0;
                    ...
                    <TableHead>
                        <TableRow>
                            <TableCell component="tr" scope="row" align="left">
                                <b style={{ color: '#306754' }}>Nome</b>
                            </TableCell>
                            <TableCell component="tr" scope="row" align="right">
                                <b style={{ color: '#306754' }}>Valor</b>
                            </TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {(rowsPerPage > 0
                            ? creditos.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                            : creditos
                        ).map((row) => (
                            <TableRow key={row.id}>
                                <TableCell component="th" scope="row" align="left">
                                    <Box sx={{
                                        ...
                                        <IconButton
                                            ...
                                            sx={{ marginRight: 1 }}
                                            onClick={(ev) => {
                                                dispatch(deletarCredito(row.id))
                                                props.handleRefresh()
                                            }}
                                        >
